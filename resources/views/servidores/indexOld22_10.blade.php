            ...
            <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
                ...
                <div class="p-6 border-b border-gray-100">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <!-- Versão mais segura -->
                        {{-- <div class="text-sm text-gray-500"
                            x-text="`${filtrados()?.length || 0} de ${servidores?.length || 0} servidores`">
                        </div> --}}
                    </div>
                </div>
                                ...
                                <tr class="transition-colors duration-150 hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <!-- Botão Ver Férias - CORRIGIDO -->
                                            <a x-bind:href="urlFerias(s.id)"
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('filtroServidores', () => ({
                ...
                // Método seguro para o contador
                contadorServidores() {
                    const total = this.servidores?.length || 0;
                    const filtrados = this.filtrados()?.length || 0;
                    return `${filtrados} de ${total} servidores`;
                },
                ...
                filtrados() {
                    ...
                    if (!Array.isArray(this.servidores)) {
                        console.warn('servidores não é um array:', this.servidores);
                        return [];
                    }

                    return this.servidores.filter(s => {
                        // Verificações seguras
                        const nome = (s.nome || '').toLowerCase().normalize("NFD").replace(
                            /[\u0300-\u036f]/g, "");
                        const cpf = s.cpf || '';
